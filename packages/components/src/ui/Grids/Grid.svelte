...
<script lang="ts">
  ...
  // If `contain` is true, then the grid will be centered, have a max-width of --width-lg, and the top and bottom margins will be negated (i.e. essentially removed). If `contain` is false, then the grid will stretch to fill the available space and the margin will be set to negative values to remove the gaps from the outer container.
  const style = contain ? `max-width: var(--width-lg); margin: -${gap.r / 2}px auto;` : `margin: -${gap.r / 2}px -${gap.c / 2}px`;
</script>
